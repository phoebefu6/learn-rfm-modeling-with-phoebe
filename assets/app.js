/* learn-data-thinking-with-phoebe - shared page behavior
...
(function () {
  ...
  if (crumb && mastheadWrap) {
    ...
    if (m) {
      ...
      var pages = ["01-three-numbers-and-a-snapshot.html", "02-the-cut-decides-the-segment.html", "03-the-rfm-bench.html", "04-beyond-purchase.html", "05-from-segment-to-action.html", "06-keep-it-alive.html"];
...
(function () {
  "use strict";
  var PASSPORT_KEY = "lwp-passport:rfm-modeling";
  ...
  var FNS = {
    ...
    contrast: function (s) {
      ...
      if (hx.length < 2) return { error: "Give me two hex colours, for example  #FFFFFF #7A6570" };
      function lum(h) {
        h = h.replace("#", "");
  ...
  document.querySelectorAll(".scorecard").forEach(function (sc) {
    ...
    /* Coaching lines default to the halfway grade. A final ship-gate scorecard overrides them
     * with data-msgs="full|strong|half|low" so the copy matches the strictness of the gate. */
    var MSGS = (sc.dataset.msgs || "").split("|");
    if (MSGS.length !== 4) {
      MSGS = ["Every row solid. Your artifact is further along than most people get.",
              "Strong. Take the weakest row into the next session and fix that one thing.",
              "Halfway is exactly where this should be. Pick the two lowest rows and only those.",
    ...
    rows.forEach(function (r, ri) {
      ...
      [["0", "Not yet"], ["1", "Partly"], ["2", "Solid"]].forEach(function (o, i) {
        ...
        b.addEventListener("click", function () {
          ...
          msg.textContent = t === max ? MSGS[0] : t >= max * 0.7 ? MSGS[1] : t >= max * 0.4 ? MSGS[2] : MSGS[3];
        });
        opts.appendChild(b);
      });
      r.appendChild(opts);
  ...
  /* Two tracks of six share one 12-stamp passport: designer 1-6, delivery 7-12. */
  ...
  if (strip) {
    ...
    var note = el("p", "pp-note", ppCount(got2) === 0
      ...
      : ppCount(got2) >= TOTAL_SESSIONS
        ? "Six of six. You finished the whole course - every chart you make now has a job and tells the truth."
